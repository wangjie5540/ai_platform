...
@mount_data_pv
def build_weather_feature_daily(sdate, edate, col_key, col_weather_list, dict_agg_func, image_tag="latest"):
    """
    大单过滤
    ...
    """
    return dsl.ContainerOp(name="build_weather_feature_daily",
                           ...
                           arguments=['/data/entrypoint.sh','spark-submit', '--master', 'yarn', '--deploy-mode', 'cluster', '--conf',
                                      ...
                                      'forecast/feature_processing/runnable/build_weather_feature_daily.py', str(sdate),
                                      str(edate),col_key, col_weather_list, dict_agg_func])
