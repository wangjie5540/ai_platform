...
@mount_data_pv
def build_date_feature_daily(sdate, edate, col_list, col_time, image_tag="latest"):
    """
    大单过滤
    ...
    """
    return dsl.ContainerOp(name="build_date_feature_daily",
                           ...
                           arguments=['spark-submit', '--master', 'yarn', '--deploy-mode', 'cluster', '--conf',
                                      ...
                                      'forecast/feature_processing/runnable/build_date_feature_daily.py', str(sdate),
                                      str(edate),list(col_list),str(col_time)])
