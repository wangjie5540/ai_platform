...
@mount_data_pv
def build_sales_feature_daily(sdate, edate, col_qty, image_tag="latest"):
    """
    大单过滤
    ...
    """
    return dsl.ContainerOp(name="build_sales_feature_daily",
                           ...
                           arguments=['/data/entrypoint.sh','spark-submit', '--master', 'yarn', '--deploy-mode', 'cluster', '--conf',
                                      ...
                                      'forecast/feature_processing/runnable/build_sales_feature_daily.py', str(sdate),
                                      str(edate),str(col_qty)])
