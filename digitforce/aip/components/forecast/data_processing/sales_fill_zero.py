...
@mount_data_pv
def sales_fill_zero(sdate, edate, col_openinv, col_qty, join_key, fill_value, image_tag="latest"):
    """
    ...
    """
    return dsl.ContainerOp(name="sales_fill_zero",
                           ...
                           arguments=['spark-submit', '--master', 'yarn', '--deploy-mode', 'cluster', '--conf',
                                      ...
                                      'forecast/data_processing/runnable/sales_fill_zero.py', str(sdate),
                                      str(edate), col_openinv, col_qty, join_key, fill_value])
