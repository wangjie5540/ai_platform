...
@mount_data_pv
def bincut_op(input_file, output_file, bins, vars, target,
                                 image_tag="latest"):
    """
    ...
    :param vars: 需要分箱的变量名列表,以‘，’分割， eg ‘age,quantities,times'
    :param target: 目标列Y的列名
    :param image_tag:
    :return:
    """
    return dsl.ContainerOp(name="bincut",
                           image=f"{AI_PLATFORM_IMAGE_REPO}"
                                 f"/src-data_preprocess-bincut"
                                 f":{image_tag}",
                           command="python",
