...
def sample_create(event_table_name, event_columns, item_table_name, item_columns, event_code, category_a, train_period, predict_period):
    ...
    one_year_ago = time_helper.n_days_ago_str(365)
    ...
    date_mid_str = (time_helper.get_day_datetime(max_trade_date, DATE_FORMAT) - datetime.timedelta(days=interval)).strftime(DATE_FORMAT)
    ...
    # 修改高潜候选人的圈选逻辑：该时间段未购买过该品类的用户->该时间段所有活跃用户
    sample_a_rdd = sample_a.rdd\
        .map(lambda x: (x[0], 0))\
        .reduceByKey(lambda x, y: x + y)
    sample_b_rdd = sample_b.filter(((sample_a[trade_type] == event_code) & (sample_a[fund_type] == category_a))).rdd\
        .map(lambda x: (x[0], 0))\
        .reduceByKey(lambda x, y: x + y)
