...
from ml_model.sp.predict_sp import method_called_predict_sp,get_default_conf
...
from ml_model.sp.train_sp import method_called_train_sp

def method_called_back_sp(data,key_cols,apply_model_index,forcast_start_date,predict_len,param,hdfs_path,step_len):
    """
    模型回测
    :param data: 样本
...
def back_test_sp(param,spark):
    ...
    try:
        ...
        data_pred=method_called_back_sp(data_train,key_cols,apply_model_index,forcast_start_date,predict_len,param, hdfs_path,step_len)
    except Exception as e:
        data_pred=None
        status=False
        logger_info.info(traceback.format_exc())
