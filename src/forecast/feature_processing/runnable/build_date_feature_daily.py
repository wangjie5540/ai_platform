...
def load_params(sdate, edate, col_list, col_time):
    """运行run方法时"""
    param_cur = {

        'sdate': sdate, #'20210101',
        'edate': edate, #'20220101',
        'col_list': col_list, #['dt', 'week', 'year', 'week_day', 'is_holiday'],
...
def run(sdate, edate, col_list, col_time,spark):
    ...
    setup_logging(info_log_file="build_date_feature_daily.info", error_log_file="", info_log_file_level="INFO")
    logging.info("LOADING···")
    param = load_params(sdate, edate, col_list, col_time)
...
if __name__ == "__main__":
    ...
    sdate, edate, col_list, col_time = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
    run(sdate, edate, col_list, col_time,spark)
