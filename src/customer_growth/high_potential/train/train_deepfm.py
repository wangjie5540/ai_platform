...
def train(input_params:Dict, path):
    try:
        ...
        dataset, cats_num = create_data.ConstructFeatures(
        ...
        if (len(dataset) == 0) or (len(dataset[dataset['label']==0]) == len(dataset)) or (len(dataset[dataset['label']==1]) == len(dataset)) :
            ...
            train_loader, valid_loader, cate_fea_nuniqs, nume_fea_size = process_dataset(dataset, 'label')
            ...
            fname=str(input_params['taskid'])+r".pth"
            filepath = os.path.join(outputpath,fname)

            auc = train_and_eval(model, train_loader, valid_loader, epoches, device, optimizer, loss_fcn, scheduler, filepath)
            ...
            model_target_file_path = os.path.join('hdfs:///usr/algorithm/cd/gaoqian/model', str(input_params['taskid'])+'.pth')
