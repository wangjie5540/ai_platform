...
def spark_read(spark, table_name, tempViewName, partitionField, minTime, maxTime):
    ...
    df = spark.read.format('jdbc').option('dbtable', f'{table_name}').option(
        'url', 'jdbc:mysql://172.21.32.16:9030').option('user', 'root').option('password', '').load()
    df.filter((df[partitionField] >= minTime) & (df[partitionField] <= maxTime)).write.saveAsTable(tempViewName,
                                                                                                   mode="overwrite")
    logging.info(f'success creating temp hive table: {tempViewName}')
