...
class BaseTower(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        val_tensor_data = Data.TensorDataset(torch.from_numpy(
            np.concatenate(val_x, axis=-1)), torch.from_numpy(val_y))
        val_loader = DataLoader(dataset=val_tensor_data, shuffle=shuffle, batch_size=batch_size)
        sample_num_val = len(val_tensor_data)
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            val_loss = self.validate(model, val_loader, verbose, optim, loss_func, sample_num_val)
            print("Epoch {}, Val Loss:{:.4f}".format(epoch + 1, val_loss))

            if val_loss < self._best_val_loss:
                print("val_loss is on the decline...")
            ...
            if verbose > 0:
                ...
                for name in self.metrics:
                    ...
                    eval_str += '\n' + "  - " + name + ": {0: .4f} ".format(epoch_logs[name]) + \
                                ' ' * (20 - len(name + ": {0: .4f} ".format(epoch_logs[name]))) \
                                + " - " + "val_" + name + ": {0: .4f}".format(epoch_logs["val_" + name])
                print(eval_str)
            if self.stop_training:
                break

    def validate(self, model, val_loader, verbose, optim, loss_func, sample_num_val):
        model.eval()
        with torch.no_grad():
            loss_epoch = 0
            total_loss_epoch = 0
            with tqdm(enumerate(val_loader), disable=verbose != 1) as t:
                for _, (x_val, y_val) in t:
                    x = x_val.to(self.device).float()
                    y = y_val.to(self.device).float()
                ...
                # 计算损失
                val_loss = total_loss_epoch / sample_num_val
                return val_loss
