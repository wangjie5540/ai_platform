...
#             print(cur_str)
            ...
            samples = get_samples_train(spark, order_table, odinfo, item_table, itinfo, cur_str, train_date, cat_list, predict_date)
#             print(samples)
        else:
...
#             print(feature_dates, train_date, predict_date )
            samples = get_samples_predict(spark, order_table, odinfo, item_table, itinfo, cat_list, train_date, cur_str, where_sql, user_table, usinfo)
#             print(samples)
        if len(samples)==0:
            spark.stop()
            return samples
...
def get_samples_train(spark, order_table, odinfo_map, item_table, itinfo_map, cur_str, train_date, cate_list, predict_date):
    ...
    '''.format(odinfo_map['user_id'],odinfo_map['sku'],order_table,odinfo_map['order_time'],train_date,cur_str,itinfo_map['sku'],itinfo_map['cate'],item_table,itinfo_map['dt'],cate_list,predict_date)
#     print(sql)
    samples = spark.sql(sql).toPandas()
    return samples

def get_samples_predict(spark, order_table, odinfo_map, item_table, itinfo_map, cate_list, train_date, cur_str, where_sql, user_table, usinfo):
    ...
    sql = '''
        ...
        group by a.user_id    
    '''.format(odinfo_map['user_id'],odinfo_map['sku'],order_table,odinfo_map['order_time'],train_date,cur_str,itinfo_map['sku'],item_table,itinfo_map['dt'],itinfo_map['cate'],cate_list)
#     print(sql)
    samples = spark.sql(sql).toPandas()
    if where_sql:
        sql_where = '''
...
#         print(last_x_days[i])
        ...
        sql_orders_count = '''
            ...
            group by {0}
        '''.format(odinfo_map['user_id'],odinfo_map['order_id'],'od_cat_ct_'+last_x_days_name[i],last_x_days[i]/2,odinfo_map['order_time'],'avg_cat_jg_'+last_x_days_name[i],order_table,last_x_day,cur_str,odinfo_map['sku'])
#         print(sql_orders_count)
        ...
        orders_count_df[['od_cat_ct_'+last_x_days_name[i],'avg_cat_jg_'+last_x_days_name[i]]] = orders_count_df[['od_cat_ct_'+last_x_days_name[i], 'avg_cat_jg_'+last_x_days_name[i]]].astype(np.float)
        ...
        if odinfo_map['sale_quantity']: 
            ...
            sql_order_qty = '''
                ...
                where {6} between '{7}' and '{8}'
                    and {9} in (select sku from item_cat_tmp)
                group by {0}
            '''.format( odinfo_map['user_id'], odinfo_map['sale_quantity'],'qty_cat_'+last_x_days_name[i], odinfo_map['order_id'],'qty_cat_avg_'+last_x_days_name[i], order_table, odinfo_map['order_time'],last_x_day, cur_str, odinfo_map['sku'])
#             print(sql_order_qty)
            order_qty_df =  spark.sql(sql_order_qty).toPandas()
            order_qty_df[['user_id']] = order_qty_df[['user_id']].astype(str)
            order_qty_df[['qty_cat_'+last_x_days_name[i],'qty_cat_avg_'+last_x_days_name[i]]] = order_qty_df[['qty_cat_'+last_x_days_name[i],'qty_cat_avg_'+last_x_days_name[i]]].astype(np.float) 
        ...
        if odinfo_map['sale_amount']: 
            ...
            sql_order_amt = '''
                ...
                where {6} between '{7}' and '{8}'
                    and {9} in (select sku from item_cat_tmp)
                group by {0}
            '''.format( odinfo_map['user_id'], odinfo_map['sale_amount'],'amt_cat_'+last_x_days_name[i], odinfo_map['order_id'],'amt_cat_avg_'+last_x_days_name[i], order_table, odinfo_map['order_time'],last_x_day, cur_str, odinfo_map['sku'])
#             print(sql_order_amt)
            order_amt_df = spark.sql(sql_order_amt).toPandas()
            order_amt_df[['user_id']] = order_amt_df[['user_id']].astype(str)
            order_amt_df[['amt_cat_'+last_x_days_name[i],'amt_cat_avg_'+last_x_days_name[i]]] = order_amt_df[['amt_cat_'+last_x_days_name[i],'amt_cat_avg_'+last_x_days_name[i]]].astype(np.float) 
...
#             print(sql_order_lastbuy)
        ...
        sql_orders_count_all = '''
            ...
            where {4} between '{7}' and '{8}'
            group by {0}
        '''.format( odinfo_map['user_id'], odinfo_map['order_id'],'od_ct_'+last_x_days_name[i],last_x_days[i]/2, odinfo_map['order_time'],'avg_jg_'+last_x_days_name[i], order_table,last_x_day, cur_str)
        ...
        orders_count_all_df[['od_ct_'+last_x_days_name[i], 'avg_jg_'+last_x_days_name[i]]] = orders_count_all_df[['od_ct_'+last_x_days_name[i], 'avg_jg_'+last_x_days_name[i]]].astype(np.float)
        if orders_df.empty:       
        ...
        if  odinfo_map['sale_quantity']: 
            ...
            sql_order_qty_all = '''
                ...
                where {6} between '{7}' and '{8}'
                group by {0}
            '''.format( odinfo_map['user_id'], odinfo_map['sale_quantity'],'qty_'+last_x_days_name[i], odinfo_map['order_id'],'qty_avg_'+last_x_days_name[i], order_table, odinfo_map['order_time'],last_x_day, cur_str)
            order_qty_all_df =  spark.sql(sql_order_qty_all).toPandas()
            order_qty_all_df[['user_id']] = order_qty_all_df[['user_id']].astype(str)
            order_qty_all_df[['qty_'+last_x_days_name[i],'qty_avg_'+last_x_days_name[i]]] = order_qty_all_df[['qty_'+last_x_days_name[i],'qty_avg_'+last_x_days_name[i]]].astype(np.float) 
        ...
        if  odinfo_map['sale_amount']: 
            ...
            sql_order_amt_all = '''
                ...
                where {6} between '{7}' and '{8}'
                group by {0}
            '''.format( odinfo_map['user_id'], odinfo_map['sale_amount'],'amt_'+last_x_days_name[i], odinfo_map['order_id'],'amt_avg_'+last_x_days_name[i], order_table, odinfo_map['order_time'],last_x_day, cur_str)
            order_amt_all_df =  spark.sql(sql_order_amt_all).toPandas()
            order_amt_all_df[['user_id']] = order_amt_all_df[['user_id']].astype(str)
            order_amt_all_df[['amt_'+last_x_days_name[i],'amt_avg_'+last_x_days_name[i]]] = order_amt_all_df[['amt_'+last_x_days_name[i],'amt_avg_'+last_x_days_name[i]]].astype(np.float) 
    ...
    for i in range(len(feature_dates)):
        ...
        if bh_code_map['click']:
            sql_dianji_cat = '''
                ...
                group by {0}
            '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'dianji_cat_'+last_x_days_name[i], behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['click'], bhinfo_map['sku'])
            ...
            sql_dianji_all = '''
                ...
                where
                    ...
                    and {1}='{7}' -- event_code
                group by {0}
            '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'dianji_all_'+last_x_days_name[i], behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['click'])
...
#             print(sql_dianji_all)
            ...
            dianji_cat_df[['dianji_cat_'+last_x_day]] = dianji_cat_df[['dianji_cat_'+last_x_days_name[i]]].astype(np.int)
            dianji_all_df = spark.sql(sql_dianji_all).toPandas()
            dianji_all_df[['user_id']] = dianji_all_df[['user_id']].astype(str)
            dianji_all_df['dianji_all_'+last_x_day] = dianji_all_df[['dianji_all_' + last_x_days_name[i]]].astype(np.int)
        ...
        if bh_code_map['cart_add']:
            sql_jiagou_cat = '''
                ...
                where
                    ...
                    and {8} in (select sku from item_cat_tmp) -- sku
                group by {0}
            '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'jiagou_cat_'+last_x_days_name[i], behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['cart_add'], bhinfo_map['sku'])
            sql_jiagou_all = '''
                select
                    {0} as user_id,
                    count({1}) as {2}
                ...
                where
                    ...
                    and {1}='{7}' -- event_code
               group by {0}
            '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'jiagou_all_'+last_x_days_name[i], behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['cart_add'])
...
#             print(sql_jiagou_all)
            ...
            jiagou_cat_df[['jiagou_cat_'+last_x_day]] = jiagou_cat_df[['jiagou_cat_'+last_x_days_name[i]]].astype(np.int)
            jiagou_all_df = spark.sql(sql_jiagou_all).toPandas()
            jiagou_all_df[['user_id']] = jiagou_all_df[['user_id']].astype(str)
            jiagou_all_df['jiagou_all_'+last_x_day] = jiagou_all_df[['jiagou_all_' + last_x_days_name[i]]].astype(np.int)
        ...
        if bh_code_map['browse']:
            sql_view_cat = '''
                ...
                where
                    ...
                    and {8} in (select sku from item_cat_tmp) -- sku
                group by {0}
            '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'view_cat_'+last_x_days_name[i], behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['browse'], bhinfo_map['sku'])
            sql_view_all = '''
                select
                    {0} as user_id,
                    count({1}) as {2}
                ...
                where
                    ...
                    and {1}='{7}' -- event_code
                group by {0}    
            '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'view_all_'+last_x_days_name[i], behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['browse'])
