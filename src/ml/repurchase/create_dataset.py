...
class CreateDataset:
    ...
    def ConstructFeatures(self,
        ...
        spark = SparkEnv('Repurchase Predicting').spark
        # spark = None
        ...
        try:
            if is_train:
                ...
                if predict_period == '未来15天':
                    ...
                    t2 = 60
                interval1 = max(t1, t2)
                cur_str = (datetime.datetime.strptime(cur_str, '%Y-%m-%d') - datetime.timedelta(
                    days=interval1)).strftime('%Y-%m-%d')
                #             print(cur_str)
                ...
                samples = get_samples_train(spark, order_table, odinfo, item_table, itinfo, cur_str, train_date,
                                            cat_list, predict_date)
            #             print(samples)
            else:
            ...
            logging.info('Raise error when Constructing samples and features')
            spark.stop()
            return pd.DataFrame()
...
def get_min_date_of_4_table(spark, user_table: str, order_table: str, item_table: str, bh_table: str, usinfo: Dict,
    ...
    spark_read(spark, user_table, 'user_table_tmp', 'dt', '2022-06-24', '2022-06-24')
    spark_read(spark, order_table, 'order_table_tmp', 'dt', '2022-06-23', '2022-06-23')
    spark_read(spark, item_table, 'item_table_tmp', 'dt', '2022-06-23', '2022-06-23')
    spark_read(spark, bh_table, 'bh_table_tmp', 'dt', '2022-06-23', '2022-06-23')
    order_table, user_table, item_table, bh_table = 'order_table_tmp', 'user_table_tmp', 'item_table_tmp', 'bh_table_tmp'
    sql1 = '''select max(DATE_FORMAT({0}, 'yyyy-MM-dd')) as us_max_date from {1} '''.format(usinfo['dt'], user_table)
    sql2 = '''select max(DATE_FORMAT({0}, 'yyyy-MM-dd')) as od_max_date from {1} '''.format(odinfo['order_time'],
                                                                                            order_table)
    sql3 = '''select max(DATE_FORMAT({0}, 'yyyy-MM-dd')) as it_max_date from {1} '''.format(itinfo['dt'], item_table)
    sql4 = '''select max(DATE_FORMAT({0}, 'yyyy-MM-dd')) as bh_max_date from {1} '''.format(bhinfo['event_time'],
                                                                                            bh_table)
...
def get_time_params(cur_str: str, train_period: str, predict_period: str):
    ...
    last_3_days = (current_day - datetime.timedelta(days=3)).strftime("%Y-%m-%d")
    last_7_days = (current_day - datetime.timedelta(days=7)).strftime("%Y-%m-%d")
    last_15_days = (current_day - datetime.timedelta(days=15)).strftime("%Y-%m-%d")
    last_1_month = (current_day - datetime.timedelta(days=30)).strftime("%Y-%m-%d")
    last_2_month = (current_day - datetime.timedelta(days=60)).strftime("%Y-%m-%d")
...
def get_samples_train(spark, order_table, odinfo_map, item_table, itinfo_map, cur_str, train_date, cate_list,
    ...
    sql = '''
        select 
            a.user_id,
            if(b.user_id is null, 0, 1) as lab
        from
        (
            ...
            from
                {2}
            where
                DATE_FORMAT({3}, 'yyyy-MM-dd') between '{4}' and '{5}'
            group by {0},{1})as c
            left join
            (select 
                {6} as sku,
            ...
            from 
                {8}
            where {9} in (select max({9}) from {8})
                and {7} in {10})as d
            on c.sku=d.sku
        ...
        (   
            ...
            from
                {2}
            where
                DATE_FORMAT({3},'yyyy-MM-dd') > '{5}' and DATE_FORMAT({3},'yyyy-MM-dd') <= '{11}'
            group by {0}, {1})as e
            left join
            (select 
                {6} as sku,
...
def get_order_features(spark, feature_dates: List[str], cur_str, order_table, odinfo_map, item_table, itinfo_map,
    ...
    sql_item = '''
        ...
        from
            {1}
        where
            {2} in (select max({2}) from {1})
            and {3} in {4}
    '''.format(itinfo_map['sku'], item_table, itinfo_map['dt'], itinfo_map['cate'], cate_list)
    #     print(sql_item)
...
select
                ...
                if(count(distinct {1})=1, {3}, round(datediff(from_unixtime(unix_timestamp(max(DATE_FORMAT({4},'yyyy-MM-dd')),"yyyy-MM-dd"),"yyyy-MM-dd"),from_unixtime(unix_timestamp(min(DATE_FORMAT({4},'yyyy-MM-dd')),"yyyy-MM-dd"),"yyyy-MM-dd"))/(count(distinct DATE_FORMAT({4},'yyyy-MM-dd'))-1),2)) as {5} --时间分区可能会变
            from {6}
            where DATE_FORMAT({4},'yyyy-MM-dd') between '{7}' and '{8}'
        ...
        if odinfo_map['sale_quantity']:
            ...
            sql_order_qty = '''
                ...
                from {5}
                where DATE_FORMAT({6},'yyyy-MM-dd') between '{7}' and '{8}'
                    and {9} in (select sku from item_cat_tmp)
                group by {0}
        ...
        if i == 0:
            ...
            sql_order_lastbuy = '''
                select
                    {0} as user_id,
                    if(count(distinct DATE_FORMAT({1},'yyyy-MM-dd'))=0, 60, round(datediff(from_unixtime(unix_timestamp("{2}","yyyy-MM-dd"),"yyyy-MM-dd"),from_unixtime(unix_timestamp(max(DATE_FORMAT({1},'yyyy-MM-dd')),"yyyy-MM-dd"),"yyyy-MM-dd")),2)) as lastbuy_diff
                from {3}
                where DATE_FORMAT({1},'yyyy-MM-dd') between '{4}' and '{2}'
...
def get_behavior_features(spark, feature_dates: List[str], item_table, itinfo_map, behavior_table, bhinfo_map,
    ...
    for i in range(len(feature_dates)):
        last_x_day = feature_dates[i]
        if 'click' not in bh_code_map:
            logging.info('Construct behavior features: Event click not find!')
        else:
            if bh_code_map['click']:
                sql_dianji_cat = '''
                    ...
                    from
                        {3}
                    where
                        DATE_FORMAT({4},'yyyy-MM-dd') between '{5}' and '{6}' -- Time
                        and {1}='{7}' -- event_code
                ...
                dianji_cat_df[['dianji_cat_' + last_x_day]] = dianji_cat_df[
                    ['dianji_cat_' + last_x_days_name[i]]].astype(np.int)
        ...
        if 'cart_add' not in bh_code_map:
            logging.info('Construct behavior features: Event cart_add not find!')
        else:
            if bh_code_map['cart_add']:
                ...
                '''.format(bhinfo_map['user_id'], bhinfo_map['event_code'], 'jiagou_cat_' + last_x_days_name[i],
                           behavior_table, bhinfo_map['event_time'], last_x_day, cur_str, bh_code_map['card_add'],
                           bhinfo_map['sku'])
                sql_jiagou_all = '''
                    select
                        {0} as user_id,
                ...
                jiagou_cat_df[['jiagou_cat_' + last_x_day]] = jiagou_cat_df[
                    ['jiagou_cat_' + last_x_days_name[i]]].astype(np.int)
        ...
        if 'browse' not in bh_code_map:
            logging.info('Construct behavior features: Event browse not find!')
        else:
            if bh_code_map['browse']:
                ...
                view_cat_df[['view_cat_' + last_x_day]] = view_cat_df[['view_cat_' + last_x_days_name[i]]].astype(
                    np.int)
...
def get_label_features(spark, samples: pd.DataFrame, user_table, usinfo_map, cur_str):
        ...
        sql_gender = '''
            ...
            left join
                (select 
                    ...
                    {1} as gender 
                from {2}
                --where DATE_FORMAT({3},'yyyy-MM-dd') = '{4}' 
                ) as b
            on a.user_id = b.user_id
        ...
        sql_age = '''
            ...
            left join
                (select 
                    ...
                    {1} as age 
                from {2}
                where DATE_FORMAT({3},'yyyy-MM-dd') = '{4}' ) as b
            on a.user_id = b.user_id
    ...
    if ('consume_level' in usinfo_map) and usinfo_map['consume_level'] != '':
        sql_consume_lvl = '''
            select 
                a.user_id,
                b.consume_lvl
    ...
    if ('online_signup_time' in usinfo_map) and usinfo_map['online_signup_time'] != '':
        sql_sign_on = '''
            ...
            left join
                (select 
                    {0} as user_id,
                    if({1} is null, 0, round(datediff(from_unixtime(unix_timestamp("{2}","yyyy-MM-dd"),"yyyy-MM-dd"),from_unixtime(unix_timestamp(DATE_FORMAT({1},'yyyy-MM-dd'),"yyyy-MM-dd"),"yyyy-MM-dd")),0)) as sign_on_days 
                from {4}
                where DATE_FORMAT({3},'yyyy-MM-dd') = '{2}' ) as b
            on a.user_id = b.user_id
    ...
    if ('recent_view_day' in usinfo_map) and usinfo_map['recent_view_day'] != '':
        sql_latest_view = '''
            ...
            left join
                (select 
                    {0} as user_id,
                    if({1} is null, 0, round(datediff(from_unixtime(unix_timestamp("{2}","yyyy-MM-dd"),"yyyy-MM-dd"),from_unixtime(unix_timestamp(DATE_FORMAT({1},'yyyy-MM-dd'),"yyyy-MM-dd"),"yyyy-MM-dd")),0)) as latest_view_days
                from {4}
                where DATE_FORMAT({3},'yyyy-MM-dd') = '{2}' 
               ) as b
            on a.user_id = b.user_id
