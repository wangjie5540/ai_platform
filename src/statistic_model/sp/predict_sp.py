...
from digitforce.aip.sof.common.config import get_config
...
from src.statistic_model.sp.data_prepare import *
from src.time_series.model.time_series_predict import model_predict
...
def get_default_conf():
    ...
    :return: 默认参数
    """
    file_tmp="/statistic/config/"
    statistic_model_operation=file_tmp+'statistic_model_operation.toml'
    statistic_model=file_tmp+r'statistic_model.toml'
    ...
    if os.path.exists(file_path):#如果压缩文件存在，是为了兼顾spark_submit形式
        try:
            ...
            statistic_model_operation=dst_dir+statistic_model_operation  # 解压后的地址
            statistic_model=dst_dir+statistic_model  # 解压后的地址
            sales_data_file=dst_dir+sales_data_file
        except:
            statistic_model_operation=file_path+statistic_model_operation  # 解压后的地址
            statistic_model=file_path+ statistic_model#解压后的地址
            sales_data_file = file_path + sales_data_file

    conf_default=get_config(statistic_model_operation,'default')
    method_param_all=get_config(statistic_model)#模型参数
...
def predict_sp(param,spark):
    ...
    logger_info.info("statistic_model_operation:")
    logger_info.info(str(param))
    mode_type=param['mode_type']
    spark_inner=0
    if str(mode_type).lower() == 'sp' and spark == None:
        try:
            add_file='statistic_model.zip'
