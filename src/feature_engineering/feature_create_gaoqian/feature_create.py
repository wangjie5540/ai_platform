...
def feature_create(sample_table_name,
    ...
    custom_label = spark_client.get_session(). \
        sql("select custom_id, replace(date,'-','') , label from {}".format(sample_table_name)). \
    ...
    x[1][0][1], x[1][0][2], x[1][0][3], x[1][0][4], x[1][0][5], x[1][0][6], x[1][0][7], x[1][0][8], x[1][1])) if x[1][
        1] else \
        ((x[0], x[1][0][0]), (
    ...
    # 3.7 将枚举型转为数值型
    merge_feature7 = merge_feature6.map(
        lambda x: (x[0], x[1][:49] + [dict_sex.get(x[1][49], 0), dict_city.get(x[1][50], 0),
