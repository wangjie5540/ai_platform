...
class EmbeddingLookupSparse(Layer):
    ...
    def call(self, inputs, **kwargs):
        if self.has_weights:
            ids, val = inputs
            combiner_embed = tf.nn.embedding_lookup_sparse(self.embed, sp_ids=ids, sp_weights=val,
                                                           combiner=self.combiner)
        else:
            combiner_embed = tf.nn.embedding_lookup_sparse(self.embed, sp_ids=inputs, sp_weights=None,
                                                           combiner=self.combiner)
