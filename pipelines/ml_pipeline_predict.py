...
def ml_predict_pipeline(train_data_start_date_str, train_data_end_date_str, run_datetime_str,
                                             solution_id, instance_id):
    # run_datetime_str = to_date_str(parse_date_str(run_datetime_str))

    show_and_action_table_maker = big_order_filter.forecast_big_order_filter(train_data_start_date_str,
                                                                             train_data_end_date_str)
    show_and_action_table_maker.container.set_image_pull_policy("Always")

    show_and_action_table_maker_1 = ml_predict.ml_model_predict(train_data_start_date_str,
                                                              train_data_end_date_str).after(show_and_action_table_maker)
    show_and_action_table_maker_1.container.set_image_pull_policy("Always")


def upload_pipeline():
    pipeline_path = "/data/aip/pipelines/" + name + ".yaml"
    kfp.compiler.Compiler().compile(ml_predict_pipeline, pipeline_path)
    client = kfp.Client()
